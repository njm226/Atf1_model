...
for i in range(len(data_pairs_sm)):
    ...
    cenH_list_m = np.zeros([len(repeat_sm),duration])
    EcoRV_list_m = np.zeros([len(repeat_sm),duration])
    ...
    for elt in range(len(repeat_sm)):
    ...
    # if  b_s >= 2.5 and b_s <= 5.9: #change later back to 3.5
...
# # save Timing list
# with open('Timing_atf1.txt', 'wb') as F:
#     pickle.dump(Timing, F)
    
    
# # save Timing list
# with open('all_criteria_noise.txt', 'wb') as F:
#     pickle.dump(all_criteria, F)
    
# # save Timing list
# with open('Differences_noise.txt', 'wb') as F:
#     pickle.dump(Differences, F)
    
# save Timing list
with open('b_list_atf1.txt', 'wb') as F:
...
#     ax.set_ylabel('S(U-->S) local rate', fontsize = 30)
#     ax.set_xlabel('S(A-->U) global rate', fontsize = 30)
...
# plt.savefig("fig_SAU_S300_1x.pdf")
